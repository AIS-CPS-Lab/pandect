...
class HoughCircleProcessor : public event_camera_codecs::EventProcessor {
   public:
    HoughCircleProcessor(
        ...
        : accumulator(image_width * image_height, 0),
        ...
        if (image_width == 0 || image_height == 0 || num_steps_angle == 0 ||
            num_steps_radius == 0 || min_radius >= max_radius) {
            std::cerr
                << "Parameters tried to initialized in HoughCircleProcessor: "
            ...
            throw std::invalid_argument(
                "Invalid parameters for HoughCircleProcessor");
        }
        radius_increment = (max_radius - min_radius) /
                           num_steps_radius;  // Calculate the radius increment
                                              // based on the number of steps
        // radius_increment = (max_radius - min_radius) /
        // static_cast<float>(num_steps_radius);
        ...
        accumulator.resize((max_radius - min_radius) / radius_increment *
                               image_width * image_height,
                           0);
    }
    ~HoughCircleProcessor() override = default;

    inline void eventCD(uint64_t, uint16_t ex, uint16_t ey,
        ...
        for (float r = min_radius; r <= max_radius; r += radius_increment) {
            for (unsigned int y = 0; y < image_height; ++y) {
                for (unsigned int x = 0; x < image_width; ++x) {
                    ...
                    if (index < 0 ||
                        index >= static_cast<int>(accumulator.size())) {
                        continue;  // Skip out of bounds
                    }
                    if (accumulator[index] > 0) {  // Threshold can be adjusted
                        detected_circles.push_back(
                            Circle{x, y, r, accumulator[index]});
                    }
                }
            }
        }
    ...
    std::vector<Circle> getBestCircles() const {
        return best_circles;  // Return the best circle found in the current
                              // packet
    }
   ...
   private:
    ...
    std::vector<std::pair<float, float>>
        unit_circle_points;  // precomputed unit circle points for radius
                             // calculations
    ...
    int getRadiusIndex(float radius) const {
        ...
        return static_cast<int>(
            (radius - min_radius) *
            num_steps_radius);  // Calculate index based on radius
    }

    int getAccumulatorIndex(float radius, unsigned int x,
                            unsigned int y) const {
        if (x >= image_width || y >= image_height) {
            throw std::out_of_range("Coordinates out of bounds");
        }
        return getRadiusIndex(radius) + y * image_width +
               x;  // Calculate index based on radius and coordinates
    }

    std::vector<Circle>
        best_circles;  // best circle found in the current packet
};
