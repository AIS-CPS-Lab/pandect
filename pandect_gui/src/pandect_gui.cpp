...
PandectGui::PandectGui() : Node("pandect_gui") {
    this->declareAndLoadParameter("param", param_, "TODO", true, false, false,
                                  0.0, 10.0, 1.0);
    this->setup();
}

template <typename T>
void PandectGui::declareAndLoadParameter(
    ...
    if (from_value.has_value() && to_value.has_value()) {
        if constexpr (std::is_integral_v<T>) {
            ...
            T step =
                static_cast<T>(step_value.has_value() ? step_value.value() : 1);
            ...
            T step = static_cast<T>(step_value.has_value() ? step_value.value()
                                                           : 1.0);
            range.set__from_value(static_cast<T>(from_value.value()))
                .set__to_value(static_cast<T>(to_value.value()))
                .set__step(step);
...
rcl_interfaces::msg::SetParametersResult PandectGui::parametersCallback(
    const std::vector<rclcpp::Parameter>& parameters) {
    for (const auto& param : parameters) {
        for (auto& auto_reconfigurable_param : auto_reconfigurable_params_) {
            if (param.get_name() == std::get<0>(auto_reconfigurable_param)) {
                std::get<1>(auto_reconfigurable_param)(param);
                RCLCPP_INFO(this->get_logger(), "Reconfigured parameter '%s'",
...
void PandectGui::setup() {
    ...
    parameters_callback_ = this->add_on_set_parameters_callback(std::bind(
        &PandectGui::parametersCallback, this, std::placeholders::_1));

    // subscriber for handling incoming messages
    subscriber_ = this->create_subscription<std_msgs::msg::Int32>(
        "~/input", 10,
        std::bind(&PandectGui::topicCallback, this, std::placeholders::_1));
    RCLCPP_INFO(this->get_logger(), "Subscribed to '%s'",
                subscriber_->get_topic_name());
}

void PandectGui::topicCallback(
...
int main(int argc, char* argv[]) {
    ...
    std::thread ros_thread(&rclcpp::executors::SingleThreadedExecutor::spin,
                           &executor);
    ros_thread.detach();
    ...
    auto camera = threepp::PerspectiveCamera::create(60, canvas.aspect(),
                                                     0.00001f, 1000.0f);
    camera->position.z = 1;
